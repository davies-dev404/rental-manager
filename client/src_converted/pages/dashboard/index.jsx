...
export default function Dashboard() {
    ...
    return (<div className="space-y-8">
      ...
      {/* KPI Cards */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Total Properties" value={stats?.totalProperties} subtext="All properties active" icon={Building2} trend="up" color="primary"/>
        <StatCard title="Occupancy Rate" value={`${stats?.occupancyRate}%`} subtext={`${stats?.vacantUnits} units vacant`} icon={Home} trend={stats?.occupancyRate && stats.occupancyRate > 80 ? 'up' : 'down'} color="emerald"/>
        <StatCard title="Monthly Revenue" value={`$${((stats?.collectedThisMonth ?? 0) / 1000).toFixed(0)}K`} subtext="+8% from last month" icon={Wallet} trend="up" color="amber"/>
        <StatCard title="Outstanding" value={`$${((stats?.outstandingAmount ?? 0) / 1000).toFixed(0)}K`} subtext="Pending collection" icon={AlertCircle} trend="down" color="rose"/>
      </div>
          ...
          <CardContent className="pl-2">
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', background: 'hsl(var(--card))' }} formatter={(value) => `$${value.toLocaleString()}`}/>
                <Legend />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 border-border/50 shadow-sm">
          ...
          <CardContent className="space-y-2">
            <RecentActivityItem icon={Wallet} title="Payment Received" description="John Doe paid rent for Unit A101 - $25,000" time="2 hours ago" status="Completed"/>
            <RecentActivityItem icon={Users} title="New Tenant Added" description="Alice Johnson registered for Unit B101" time="5 hours ago"/>
            <RecentActivityItem icon={AlertCircle} title="Overdue Rent Alert" description="Bob Smith's rent is 5 days overdue" time="1 day ago" status="Pending"/>
            <RecentActivityItem icon={Building2} title="Maintenance Requested" description="Unit C205 reported water leakage issue" time="2 days ago"/>
          </CardContent>
        </Card>
